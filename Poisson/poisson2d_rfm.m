...
% omega(omega<0) = 0;
% omega = x.*(x-b).*y.*(y-d);
om1 = x + b; om2 = y + d; om3 = b - x; om4 = d - y;
om12 = r_con(om1, om2); om34 = r_con(om3, om4);
...
figure(1),
surf(x, y, omega)
shading interp; lighting phong; axis square; axis off; camlight left
title('$\omega(x,y)$','Interpreter','latex')
% axis equal
...
%         view(36,33)
        ...
        xlabel('$x$','Interpreter','latex')
        ylabel('$y$','Interpreter','latex')
        title(['$\psi_{' int2str(k) '}=' '\omega x^{' int2str(i-1) '}y^{' int2str(j-1) '}$'],'Interpreter','latex')
    end
end
...
surf(x, y, ua(x,y))
shading interp; lighting phong; camlight left;
title('Exact solution $u(x,y)$','Interpreter','latex')
axis square
xlabel('$x$','Interpreter','latex')
ylabel('$y$','Interpreter','latex')
zlabel('$u(x,y)$','Interpreter','latex')
