...
// ToDo: modify for random bound positions
{
	...
	return (-4. + x*x)*(-4. + y*y)*(-1. + x*x + y*y);
	//return 7 - sqrt(32 - 8*pow(x,2) + pow(x,4) - 8*pow(y,2) + pow(y,4)) - 
   //sqrt(pow(-1 + pow(x,2) + pow(y,2),2) + 
     //pow(-8 + pow(x,2) + pow(y,2) + 
       //sqrt(32 - 8*pow(x,2) + pow(x,4) - 8*pow(y,2) + pow(y,4)),2));
	/*double h = (x-X0)*(x-X1)*(y-Y0)*(y-Y1);
	if(h>=10.)	return 10.;
	else 		return h;*/
}

double domegadx(double x, double y)
{
	return 2*x*(-4 + x*x)*(-4 + y*y) + 
   2*x*(-4 + y*y)*(-1 + x*x + y*y);
}

double domegady(double x, double y)
{
	return 2*(-4 + x*x)*y*(-4 + y*y) + 
   2*(-4 + x*x)*y*(-1 + x*x + y*y);
}
...
// Returns value of n-th \psi-basis function, used further, at point (x,y)
{
    ...
    //for 2nd boundary
    return chebishov_1d(x,n%N)*chebishov_1d(y,n/N)
			- omega(x,y)*(domegadx(x,y)*dchebishov_1d(x,n%N)*chebishov_1d(y,n/N)
						+domegady(x,y)*chebishov_1d(x,n%N)*dchebishov_1d(y,n/N))/
						sqrt(domegadx(x,y)*domegadx(x,y)+domegady(x,y)*domegady(x,y));
