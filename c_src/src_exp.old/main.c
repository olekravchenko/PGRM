                  ...
                  double x0, double x1, double y0, double y1, int k1, int k2)
{
	double	i,
			step = (y1-y0)/512.,
			res = 0.;
	const int   q_of_layers = (int)((y1-y0)/step) + 1;
	int         k;
	double      integrals_by_layer[q_of_layers];
	
	for(k = 0; k < q_of_layers; k++)
	{
	      integrals_by_layer[k] = 
	            Simpson_int((*f),x0,x1,y0+(double)k*step, k1,k2);
	}
...
// \int_{x0}^{x1}\int_{y0}^{y1} f(x,y)\psi_m(x,y)dydx
{
	...
	{
	      integrals_by_layer[k] = 
	            Simpson_right((*f),x0,x1,y0+(double)k*step, k1);
	}
...
// y1, y2	- sizes of rectangle by y
{
	...
	for(i = 0; i < N*N; i++)
	{
		gsl_vector_set(RightPart, i, 
		                 -integralRight(right_under_int,x1,x2,y1,y2,i));
		for(j = 0; j < N*N; j++)
		{
			gsl_matrix_set(system, i,j, 
			            integralLeft(left_under_int,x1,x2,y1,y2,i,j));
		}
	}
}
...
// from x1 till x2 by x, and from y1 till y2 by y
{
	...
	for(i=x1; i<=x2; i+=hx)
		for(j=y1; j<=y2; j+=hy)
			fprintf(op, "%f %f %f\n", i,j, 
			      fabs(reconstruct_at(solution,i,j)-u_exact(i,j)));
	fclose(op);
}
